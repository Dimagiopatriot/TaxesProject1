...
<form action="Admin" method="post">
    ...
    Робота: <input type="number" required min="0" step="0.01" name="work"
                   value="<%=retrieve.retrieveTaxPercentFromDatabase(WORK_TAX_NAME)%>"/><br/><br/>
    Авторська винагорода: <input type="number" required min="0" step="0.01" name="reward"
                                 value="<%=retrieve.retrieveTaxPercentFromDatabase(REWARD_TAX_NAME)%>"/><br/><br/>
    Продаж майна: <input type="number" required step="0.01" min="0" name="property"
                         value="<%=retrieve.retrieveTaxPercentFromDatabase(PROPERTY_TAX_NAME)%>"/><br/><br/>
    Подарунки у вигляді грошей або майна: <input type="number" required step="0.01" min="0" name="gifts"
                                                 value="<%=retrieve.retrieveTaxPercentFromDatabase(GIFTS_TAX_NAME)%>"/><br/><br/>
    Переказ коштів з-за кордону: <input type="number" required min="0" step="0.01" name="transfer"
                                        value="<%=retrieve.retrieveTaxPercentFromDatabase(TRANSFER_TAX_NAME)%>"/><br/><br/>
    Пільги на дітей: <input type="number" min="0" required step="0.01" name="children_privileges"
                            value="<%=retrieve.retrieveTaxPercentFromDatabase(CHILDREN_PRIVILEGES_TAX_NAME)%>"/><br/><br/>
    Матеріальна допомога: <input type="number" min="0" required step="0.01" name="material_aid"
